...
import { data as addData, addFromMeaningData, addFromWordData } from './commands/add';
...
(async () => {
    try {
        ...
        const guildIds = (process.env.GUILD_ID || '').split(',').map(id => id.trim()).filter(id => id);

        if (guildIds.length === 0) {
            console.warn('⚠️ GUILD_ID が設定されていません。コマンド登録をスキップします。');
            return;
        }
