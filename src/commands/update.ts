  ...
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((option) =>
        option.setName("word").setDescription("更新する単語").setRequired(true),
      ),
  )
  .addSubcommand((sub) =>
    sub
      ...
      .addStringOption((option) =>
        option
          .setName("keyword")
          .setDescription("対象を検索するキーワード（空欄なら最新25件を表示）"),
      ),
  );
...
export const updateCommand = async (
  ...
  if (!subcommand) {
    await interaction.reply({
      content:
        "⚠️ コマンドの形式が古いです!Discordを再読み込み(Ctrl+R)してください。",
...
async function handleWordUpdate(interaction: ChatInputCommandInteraction) {
  ...
  try {
    const submitted = await interaction.awaitModalSubmit({
      time: 10 * 60 * 1000,
      filter: (i) =>
        i.customId === `updateWordModal_${word.id}` &&
    ...
    await submitted.reply({
      content: `✅ **「${word.titles[0]?.text}」** の情報を更新しました！`,
    });
  } catch (error) {
    // time out
  }
  ...
  try {
    ...
    if (targets.length === 0) {
      await interaction.editReply(
        `❌ キーワード **「${keyword}」** に一致する単語は見つかりませんでした。`,
      );
      return;
    }
    ...
    const tagSelectMenu = new StringSelectMenuBuilder()
      ...
      .addOptions(
        new StringSelectMenuOptionBuilder()
          .setLabel("❌ タグを削除")
          .setValue("__REMOVE__")
          .setDescription("選択した単語のタグを外します"),
        new StringSelectMenuOptionBuilder()
          .setLabel("✨ 新しいタグを手入力")
          .setValue("__NEW__")
          .setDescription("新しいタグを作成して付与します"),
        ...existingTags
          .slice(0, 23)
          .map((t) =>
    ...
    if (selectedVal === "__NEW__") {
      ...
      await modalSubmit.editReply(
        `🎉 **${selectedWordIds.length}件** のタグを **「${finalTag}」** に一括更新しました！`,
      );
      return;
    }
