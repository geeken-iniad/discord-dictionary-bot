  ...
  try {
    ...
    const generateView = async (page: number) => {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(titleText)
        .setFooter({
          text: `ページ ${page + 1} / ${maxPage + 1} (全 ${totalCount} 件)`,
        })
        .addFields(
          currentItems.map((word) => {
            ...
            const shortMeaning =
              word.meaning.length > 50
                ...
                : word.meaning;
            const authorInfo = word.authorName
              ? `by ${word.authorName}`
              : "不明";
        ...
        if (i.isStringSelectMenu() && i.customId === "selectWordForTag") {
          ...
          try {
            const selection = await popupMessage.awaitMessageComponent({
              ...
              time: 60_000,
              filter: (subI: StringSelectMenuInteraction) =>
                subI.user.id === i.user.id,
            });
            ...
            const newTag =
              !newTagRaw || newTagRaw === "__REMOVE_TAG__" ? null : newTagRaw;
